...
window <- 1000
tis <- "Whole_Blood"
out_dir <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/comp_to_GTEx/Results/chr_Results/"

expression_RDS <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/comp_to_GTEx/expression/Whole_Blood_expression_abrv.RDS"
geno_file <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/comp_to_GTEx/genos/geno_by_chr/" %&% "genotype.chr" %&% chrom %&% ".txt"
gene_annot_RDS <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/comp_to_GTEx/gene_annotation.RDS"
snp_annot_RDS <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/comp_to_GTEx/genos/snp_annot/" %&% "snp_annot.chr" %&% chrom %&% ".RDS"
...
rownames(genotype) <- genotype$SNP
genotype <- genotype %>% select(-c(SNP, CHR, POS, COUNTED, ALT))
names(genotype) <- substring(names(genotype), 12,21)
names(genotype) <- gsub('\\.', '-', names(genotype))
genotype <- genotype[, intersect(rownames(expression), colnames(genotype))]
# Transpose genotype for glmnet
genotype <- t(genotype)
gene_annot <- readRDS(gene_annot_RDS)
gene_annot <- subset(gene_annot, gene_annot$chromosome == paste("chr", chrom, sep=""))
...
expression <- expression[match(rownames(genotype), rownames(expression)),]
...
for (i in 1:length(exp_genes)) {
  ...
  cisgenos <- na.omit(cisgenos[,minorsnps, drop = FALSE])
  if (ncol(cisgenos) < 2) {
    ...
    exppheno <- exppheno[match(rownames(cisgenos), rownames(exppheno)),]
    exp_samples <- rownames(exppheno)
    n_samples <- length(exp_samples)
    groupid <- sample(1:n_k_folds, length(exppheno), replace = TRUE)
