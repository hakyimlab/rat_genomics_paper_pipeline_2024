...
for (i in 1:length(exp_genes)) {
  ...
  if (length(bestbetas) > 0) {
    ...
    rsq <- res$r.squared
    pval <- res$coef[2,4]
    #cor <-  mean(cor(as.matrix(cisgenos), y = as.vector(exppheno))[,1])
...
write.table(resultsarray,file=out_dir %&% "TW_" %&% tis %&% "_exp_" %&% n_k_folds %&% "-foldCV_elasticNet_alpha" %&% alpha %&% "_" %&% snpset %&% ".txt",quote=F,row.names=F,sep="\t")
